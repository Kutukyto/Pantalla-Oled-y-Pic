...
//#include <htc.h>
 ...
 // Trimming values to be adjusted:  
 #define X 0 // for Delay10TCYx(2)
 #define Y 0  // for Delay10TCYx(1)
 /* * Delay multiples of 10 Tcy * Passing 0 (zero) results in a delay of 2560 cycles. */   
  void Delay10TCYx(unsigned char unit){  
 if(--unit) // compensate for excessive first pass delay  
    {  
       _delay(X); // Trim X for closest to 20 cycles total @  Delay10TCYx(2). Comment whole line if 0  
       do {   
           _delay(7);   // 10 -  3 cycles loop overhead   
       } while(--unit != 0);   
     }  
      else _delay(Y); // Trim Y for closest to 10 cycles total @ Delay10TCYx(1).  comment whole line if 0  
 }  
